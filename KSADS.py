...
def read_csv(date, form):
    return pd.read_csv(
        os.path.join(downloads_dir, date, form + ".csv"), low_memory=False
    )
...
if not deleted.empty:
    display(
        HTML(
            "<H3>Deleted Rows</H3><SMALL> The following rows have been deleted in the new data, compared to the previously available data. </SMALL>"
        )
    )
...
if not modified.empty:
    display(
        HTML(
            "<H3>Modified Rows</H3><SMALL> The following rows have been modified. </SMALL>"
        )
    )
...
dups = h.intersection(
    redcap_df, added, ["patientid", "patienttype"], sources=("current", "new")
...
def convert_to_links():
    values = [
        wg.HTML(
            f'<a target="_blank" href="https://redcap.wustl.edu/redcap/redcap_v8.11.0/DataEntry/record_home.php?pid=3355&arm=1&id={x}">view</a>'
        )
...
# %%
data["merged"] = data["merged"].drop_duplicates(
    ["patientid", "patienttype"], keep="last"
)
df = data["merged"]
